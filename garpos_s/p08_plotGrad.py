...
#	print("wh: ",scale_x, scale_y)
#	print("cp: ",mean_x, mean_y)
	out=[scale_x, scale_y, mean_x, mean_y] #,str(transf)[16:27],str(transf)[27:39],str(transf)[39:51],str(transf)[59:70],str(transf)[70:82],str(transf)[82:94]]
#	print(transf)
	return ax.add_patch(ellipse), out

def plot_residuals(resf, obsfile, mpfile, d0, MTs, scalel, grad, i00, iresf):
...
#		Correlation Matrix for '+ os.path.basename(corfile), y=1.05, size=15)
		c = patches.Circle(xy=(0, 0), radius=0.2, ec='y', fill=False)
		axE.add_patch(c)
		c = patches.Circle(xy=(0, 0), radius=0.2, ec='y', fill=False)
		axN.add_patch(c)
		for s in range(20,shot_tmp.ST.shape[0],20):
		...
		for s in range(12,20):
			if i00[s] == 100:
				if int(resf.split('.')[1]) > int(s)*100:
					i00[s]=iresf
		if iresf > 0:
			for s in range(iresf):
				if s < i00[13]:
					confidence_ellipse(grad[s][1], grad[s][0], axE, alpha=0.2, facecolor='purple')
					confidence_ellipse(grad[s][3], grad[s][2], axN, alpha=0.2, facecolor='purple')
				elif s < i00[14]:
					confidence_ellipse(grad[s][1], grad[s][0], axE, alpha=0.2, facecolor='red')
					confidence_ellipse(grad[s][3], grad[s][2], axN, alpha=0.2, facecolor='red')
				elif s < i00[15]:
					confidence_ellipse(grad[s][1], grad[s][0], axE, alpha=0.2, facecolor='orange')
					confidence_ellipse(grad[s][3], grad[s][2], axN, alpha=0.2, facecolor='orange')
				elif s < i00[16]:
					confidence_ellipse(grad[s][1], grad[s][0], axE, alpha=0.2, facecolor='yellow')
					confidence_ellipse(grad[s][3], grad[s][2], axN, alpha=0.2, facecolor='yellow')
				elif s < i00[17]:
					confidence_ellipse(grad[s][1], grad[s][0], axE, alpha=0.2, facecolor='green')
					confidence_ellipse(grad[s][3], grad[s][2], axN, alpha=0.2, facecolor='green')
				elif s < i00[18]:
					confidence_ellipse(grad[s][1], grad[s][0], axE, alpha=0.2, facecolor='cyan')
					confidence_ellipse(grad[s][3], grad[s][2], axN, alpha=0.2, facecolor='cyan')
				elif s < i00[19]:
					confidence_ellipse(grad[s][1], grad[s][0], axE, alpha=0.2, facecolor='blue')
					confidence_ellipse(grad[s][3], grad[s][2], axN, alpha=0.2, facecolor='blue')
				else:
					confidence_ellipse(grad[s][1], grad[s][0], axE, alpha=0.2, facecolor='gray')
					confidence_ellipse(grad[s][3], grad[s][2], axN, alpha=0.2, facecolor='gray')
		outE=confidence_ellipse(shot_tmp.gradV2e, shot_tmp.gradV1e, axE, edgecolor='red')
		print(resf.split('/')[1],resf.split('.')[1],"East",outE[1][0],outE[1][1],outE[1][2],outE[1][3])#,outE[1][4],outE[1][5],outE[1][6],outE[1][7],outE[1][8],outE[1][9])
		outN=confidence_ellipse(shot_tmp.gradV2n, shot_tmp.gradV1n, axN, edgecolor='red')
		print(resf.split('/')[1],resf.split('.')[1],"North",outN[1][0],outN[1][1],outN[1][2],outN[1][3])#,outN[1][4],outN[1][5],outN[1][6],outN[1][7],outN[1][8],outN[1][9])
		grad[iresf][0] = shot_tmp.gradV1e
		grad[iresf][1] = shot_tmp.gradV2e
		grad[iresf][2] = shot_tmp.gradV1n
		grad[iresf][3] = shot_tmp.gradV2n
...
if __name__ == '__main__':
	...
	parser.add_option("--dir", action="store", type="string", default="demo_prep", dest="DIR", help=u'Set site name')
	parser.add_option("--posdiff", action="store_true", default=False, dest="posd", help=u"Make pos.diff file")
	parser.add_option("--scale", action="store", type="string", default="0.20", dest="scale", help=u'Set_scale')
#	parser.add_option("--resfiles", action="store", type="string", default="", dest="resfls", nargs='*', help=u"Set res-file names (if not set, all files in -d directory are used)")
	(options, args) = parser.parse_args()
	#####################################################################

	print(options.DIR, options.SITE)
	resfiles = glob.glob('%s/%s/*res.dat' % (options.DIR, options.SITE))
	...
	grad = [pd.DataFrame([]) for i in range(len(resfiles))]
	i00 = [100 for i in range(30)]
	for resf in resfiles:
		...
		plot_residuals(resf, obsf, mpf, d0, MTs, options.scale, grad, i00, iresf)
