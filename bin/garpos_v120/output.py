...
Modified:
    02/13/2025 to add TD S/N, MT models, etc.
"""
import os
import numpy as np
...
def outresults(odir, suf, cfg, imp0, slvidx0,
    ...
    try:
        TD_SN = cfg.get("Obs-parameter", "TD_SN")
    except:
        TD_SN = "unknown/undefined"
    ...
    try:
        MT_models = cfg.get("Site-parameter", "MT_models")
    except:
        MT_models = "unknown/undefined"
    ...
    resobsp   = [site, camp, TD_SN, date0, datej, refframe, svpf]
    resdatap  = [savfile, len(shots.index), shots[~shots['flag']].index.size]
    ressitep  = [geocent[0], geocent[1], geocent[2], MT_models]
    ...
    if not "ping_id" in shots.columns:
        shots["ping_id"] = "undefined"
    ashot = shots.loc[:,[
        'SET','LN','MT','TT','ResiTT', 'TakeOff', 'Azimuth', 'gamma', 'flag',
        'ST','ant_e0','ant_n0','ant_u0','head0','pitch0','roll0',
        'RT','ant_e1','ant_n1','ant_u1','head1','pitch1','roll1', 'ping_id',
