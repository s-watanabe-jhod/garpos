...
Modified:
    ...
    02/13/2025 by S. Watanabe
        to add TD S/N, MT models, etc.
    07/18/2026
        remove sksparse dependency; use scipy cho_factor/splu
        vectorized Jacobian via BSpline.design_matrix
"""
import os
import sys
import math
...
# garpos module
from .setup_model import init_position, make_knots, first_derivative, second_derivative, data_correlation
from .forward import calc_forward, calc_gamma, calc_jcb_gamma, jacobian_pos
...
def MPestimate(cfgf, icfgf, odir, suf, lamb0, lgrad, mu_t, mu_m):
    ...
    constraint0 = int(icfg.get("HyperParameters","ConstraintType0"))
    constraintg = int(icfg.get("HyperParameters","ConstraintTypeGrad"))
    ...
    Hs = []
    if constraint0 == 1:
        H1_0 = first_derivative(knots[0], spdeg)
        Hs.append(H1_0)
    elif constraint0 == 2:
        H2_0 = second_derivative(knots[0], spdeg)
        Hs.append(H2_0)
    else:
        print("Error in setting ConstraintType0 (must be 1 or 2)")
        sys.exit(1)

    if constraintg == 1:
        H1_1 = first_derivative(knots[1], spdeg)
        H1_2 = first_derivative(knots[3], spdeg)
        Hs.append(H1_1)
        Hs.append(H1_1)
        Hs.append(H1_2)
        Hs.append(H1_2)
    elif constraintg == 2:
        H2_1 = second_derivative(knots[1], spdeg)
        H2_2 = second_derivative(knots[3], spdeg)
        Hs.append(H2_1)
        Hs.append(H2_1)
        Hs.append(H2_2)
        Hs.append(H2_2)
    else:
        print("Error in setting ConstraintTypeGrad (must be 1 or 2)")
        sys.exit(1)

    H = lil_matrix( (imp0[5]-imp0[0], imp0[5]-imp0[0]) )
    for k, lamb in enumerate(lambdas):
        if len(knots[k]) == 0:
            continue
        k0 = imp0[k]-imp0[0]
        k1 = imp0[k+1]-imp0[0]
        H[k0:k1, k0:k1] = Hs[k] / lamb
    ...
    rankDi = np.linalg.matrix_rank(Di.toarray(), tol=1.e-9)
    ...
    if rankDi != len(eigvDi):
        ...
        return ["error_in_D", np.nan, np.inf, np.array([np.nan, np.nan, np.nan])]
    ...
    if icorrE:
        try:
            ...
            logdetEi = -np.sum(np.log(np.abs(E_factor.U.diagonal())))
        except:
            print("Error in calculating log(det Ei) !!!")
            return ["error_in_E", np.nan, np.inf, np.array([np.nan, np.nan, np.nan])]
    else:
        w = TT0**2. / scale**2.
    ...
    for iloop in range(maxloop):
        ...
        if rsig > 0.1 or iloop == 0:
            jcb = np.zeros((nmp, ndata))
            jcb[nmppos:, :] = calc_jcb_gamma(tmp, imp0, spdeg, knots, scale, nmppos)
            if len(slvidx0) != 0:
                jcb0 = jacobian_pos(icfg, mp, slvidx0, tmp, mtidx, svp, T0)
                jcb[:nmppos, :] = jcb0[:nmppos, :].toarray()
        ...
        alpha = 1.0 # fixed
        if icorrE:
            EiG = E_factor.solve(jcb.T)
            AktEiAk = jcb @ EiG / scale**2.
            rk = jcb @ E_factor.solve(tmp.ResiTT.values) / scale**2. + Di_d @ (mp0-mp1)
        else:
            AktEiAk = (jcb * w) @ jcb.T
            rk  = jcb @ (tmp.ResiTT.values * w) + Di_d @ (mp0-mp1)

        Cki_d = AktEiAk + Di_d
        Cki_cf, Cki_low = cho_factor(Cki_d, lower=True, overwrite_a=True, check_finite=False)
        Ckrk = cho_solve((Cki_cf, Cki_low), rk, check_finite=False)
        dmp  = alpha * Ckrk
        ...
        if icorrE:
            misfit = rttadp @ E_factor.solve(rttadp) / scale**2.
        else:
            misfit = np.dot(rttadp**2., w)
    ...
    S = misfit + ( (mp0-mp1) @ Di_d ) @ (mp0-mp1)

    logdetCki = 2.0 * np.sum(np.log(np.diag(Cki_cf)))

    abic = dof * math.log(S) - logdetEi - logdetDi + logdetCki
    sigobs  = (S/dof)**0.5 * scale

    Ck = cho_solve((Cki_cf, Cki_low), np.eye(nmp), check_finite=False)
    C = S/dof * Ck
