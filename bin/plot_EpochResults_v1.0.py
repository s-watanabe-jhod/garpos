...
"""
Minor modifications
2023.01.18 nakamura: added pd.to_numpy() to the dataframes used for plotting results to avoid warning below:
                     /home/sgo/anaconda3/lib/python3.7/site-packages/matplotlib/axes/_base.py:239:
...
def plot_residuals(resf, obsfile, mpfile, d0, MTs, V0, ext):
    ...
    for isetg, setgroup in enumerate(setgroups):
        ...
        for i in range(len(MTs)):
            mt = MTs[i]
            xx = shot_tmp.loc[(shot_tmp['MT']==mt),'dt'].to_numpy()
            yy = shot_tmp.loc[(shot_tmp['MT']==mt),'ResiTT'].to_numpy()
            ax0.plot(xx, yy, marker=".", color=mtcmap(i), linestyle='None', label=mt)

            xr = rejected.loc[(rejected['MT']==mt),'dt'].to_numpy()
            yr = [len(MTs)-i for x in xr]
            ax1.plot(xr, yr, marker="x", color=mtcmap(i), linestyle='None', label=mt)

        ax0.plot(rejected['dt'].to_numpy(), rejected['zeros'].to_numpy(),  marker="x", color="black", linestyle='None', label=r"Rjc." )
        ...
        for s, st in enumerate(sets):
            ...
            ax0.plot(tm['dt'].to_numpy(), tm['a'].to_numpy(), color=setcmap(s), linewidth=4, zorder=1)
            ax3.plot(tm['dt'].to_numpy(), tm['zeros'].to_numpy(), color=setcmap(s), linewidth=4, zorder=1,label=st)
        ...
        for i in range(len(MTs)):
            mt = MTs[i]
            xx = shot_tmp.loc[(shot_tmp['MT']==mt),'dt'].to_numpy()
            yy = shot_tmp.loc[(shot_tmp['MT']==mt),'dV'].to_numpy()
            ll = r"%s" % mt
        ...
        for s, st in enumerate(sets):
            ...
            tm = shot_tmp[(shot_tmp['SET']==st)].reset_index(drop=True).copy()
            ax2.plot(tm.dt.to_numpy(), tm.dV0.to_numpy(), label=lb0, color='black')
        ...
        for s, st in enumerate(sets):
            ...
            tm = shot_tmp[(shot_tmp['SET']==st)].reset_index(drop=True).copy()
            ax3.plot(tm.dt.to_numpy(), tm.gradV1e.to_numpy()/scale+offset1, label=lb1e, color='mediumblue', linestyle='dashed')
            ax3.plot(tm.dt.to_numpy(), tm.gradV1n.to_numpy()/scale+offset1, label=lb1n, color='mediumblue', linestyle='solid' )
            ax3.plot(tm.dt.to_numpy(), tm.gradV2e.to_numpy()/scale+offset2, label=lb2e, color='navy', linestyle='dashed')
            ax3.plot(tm.dt.to_numpy(), tm.gradV2n.to_numpy()/scale+offset2, label=lb2n, color='navy', linestyle='solid' )
